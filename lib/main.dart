...
class _CyberGamePageState extends State<CyberGamePage> with TickerProviderStateMixin {
  ...
  double _getRotationAngle() {
    switch (currentPlayerIndex) {
      case 0: return 0.0; case 1: return math.pi / 2;
      case 2: return math.pi; case 3: return -math.pi / 2;
      default: return 0.0;
    }
  }
  ...
  void _showEditPlayerDialog(int index) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Text("ناونان", style: const TextStyle(color: Colors.white)),
        content: TextField(controller: textController, style: const TextStyle(color: Colors.white)),
        actions: [ElevatedButton(onPressed: () { if (textController.text.trim().isNotEmpty) { setState(() { players[index].name = textController.text.trim(); }); Navigator.pop(context); } }, child: const Text("OK"))],
      ),
    );
  }
  ...
  Offset _getCellCoordinates(int pos, double cSize, int pIndex) {
    ...
    return Offset(col * cSize + (pIndex % 2 == 0 ? 2.0 : cSize * 0.38), (9 - row) * cSize + (pIndex < 2 ? 2.0 : cSize * 0.38));
  }
              ...
              child: Center(
                child: GestureDetector(
                  ...
                  child: AnimatedRotation(
                    turns: _getRotationAngle() / (2 * math.pi), duration: const Duration(milliseconds: 600), curve: Curves.easeInOutCubic,
                    child: Container(
                          ...
                          child: LayoutBuilder(
                            builder: (context, constraints) {
                              ...
                              return Stack(
                                children: [
                                  GridView.builder(
                                    physics: const NeverScrollableScrollPhysics(), gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 10), itemCount: 100,
                                    itemBuilder: (context, idx) => Container(decoration: BoxDecoration(color: idx % 2 == 0 ? const Color(0xFF10121E) : const Color(0xFF07080F), border: Border.all(color: Colors.white10, width: 0.2))),
                                  ),
                                  ...
                                  ...List.generate(players.length, (index) {
                                    ...
                                    return AnimatedPositioned(
                                      duration: Duration(milliseconds: (index == currentPlayerIndex && isMoving) ? 240 : 350), curve: Curves.easeInOut, left: coords.dx, top: coords.dy, width: cSize * 0.45, height: cSize * 0.45,
                                      child: Container(decoration: BoxDecoration(shape: BoxShape.circle, color: p.color, border: Border.all(color: Colors.white))),
                                    );
                                  }),
                                ],
                              );
...
class AdvancedBoardPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ladders.forEach((s, e) {
      ...
      canvas.drawLine(pS, pE, Paint()..color = const Color(0xFFA0522D)..strokeWidth = 4.0..style = PaintingStyle.stroke);
    });
    snakes.forEach((s, e) {
      Offset pS = _getCenter(s, size), pE = _getCenter(e, size);
      canvas.drawLine(pS, pE, Paint()..color = const Color(0xFF228B22)..strokeWidth = 4.0..style = PaintingStyle.stroke);
      canvas.drawCircle(pS, 5, Paint()..color = Colors.red);
    });
